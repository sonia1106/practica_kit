	...
	>
        ...
        <button
          on:click={() => {
            ...
            closeAllExcept('inicio');

            openMenu.inicio = !openMenu.inicio;
            openMenu.inicioPinned = openMenu.inicio;
          }}
          class={`
        ...
        >
          ...
          <span class={`transition ${openMenu.inicio ? 'rotate-90' : ''}
              ${$sidebarOpen || $sidebarHover ? 'opacity-100 ml-0' : 'opacity-0 -ml-4 pointer-events-none'}
          `}>
        ...
        {#if openMenu.inicio && ($sidebarOpen || $sidebarHover) }
         <div class="ml-6 mt-1 space-y-1">
            <SidebarLink
            href="/start/companies"
            label="Empresas"
            />
          </div>
        {/if}
